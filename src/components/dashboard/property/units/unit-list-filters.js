...
export const UnitListFilter = (props) => {
  const {
    properties,
    unittypes,
    onChange,
  ...
  const conditionValues = useMemo(
    () =>
      filterItems
        .filter((filterItems) => filterItems.field === "condition")
        .map((filterItems) => filterItems.value),
    [filterItems]
  );
      ...
      filterItems.forEach((filterItem) => {
        switch (filterItem.field) {
          ...
          case "unittype":
            ...
            break;
          case "condition":
            filters.condition.push(filterItem.value);
            break;
          case "status":
  ...
  const handleConditionChange = (values) => {
    setFilterItems((prevState) => {
      ...
      values.forEach((value) => {
        if (!valuesFound.includes(value)) {
          const option = UNIT_CONDITIONS.find(
