...
import { Box, Button, CircularProgress, Container, Grid, IconButton, TextField, Typography, Card, CardContent } from '@mui/material'
...
const UnitScan = () => {
    ...
    const { mutateAsync: addTask, data: taskData, isLoading: taskLoading } = useMutation((data) =>
    ...
    const { mutateAsync: updateproduct, isLoading: isProductLoading } = useMutation((data) => {
    ...
    useEffect(() => {
        if (taskData !== undefined) {
            ...
            toast.success('Thank you for selecting your desired item, Our team will take care of the rest.')
        }
    }, [taskData])
    ...
    useEffect(() => {
        if (productData !== undefined && productIds.length > 0) {
            ...
            const filteredProducts = productData?.data?.data?.filter(item => productIds.includes(item.id))
            console.log('filteredProduct', filteredProducts);
            setFilteredProducts(filteredProducts.map(item => ({ ...item, count: 0 })))
    ...
    useEffect(() => {
        if (checklistData !== undefined) {
            const productTypeChecklist = checklistData?.data?.data?.filter(item => item.category_type === "Product")
            ...
            if (productTypeChecklist && productTypeChecklist.length > 0) {
                ...
                let productIds = productTypeChecklist.map(item => item.all_cat_ids).flat()
        ...
        <Box>
            ...
            >
                ...
                <div className={styles.mainWrapBook}>
                        ...
                        {unitId && (
                            <Box>
                                <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                                    <Image src={unitLogo} alt='unitLogo' width={50} height={50} />
                                    <Typography variant="subtitle1" textAlign={'center'} sx={{ ml: 2 }}>
                                        Enhance your stay with our convenient in room ordering services
                                    </Typography>
                                </Box>
                    ...
                    {unitId && (
                        <Box sx={{ my: 2, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                            <Typography variant="h6" sx={{ alignItems: 'center' }}>Menu of our offerings</Typography>
                        </Box>
                    )}
                            ...
                            <CardContent>
                                <Box sx={{
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            if (parseInt(e.target.value) >= 0) {
                                                ...
                                                newfilteredProducts[idx].count = parseInt(e.target.value)
                                    ...
                                    <IconButton
                                        ...
                                        sx={{ backgroundColor: "primary.main", mx: 1 }}
                                        onClick={() => {
                                ...
                                <Typography sx={{ fontSize: 16, my: 1, fontWeight: '700' }}>{elItem?.description}</Typography>
                                <TextField
                                    size="small"
                                    fullWidth
                                    label="Comment"
                                    ...
                                    onChange={(e) => {
                                        ...
                                        const newfilteredProducts = [...filteredProducts]
                                        newfilteredProducts[idx].user_comment = e.target.value
                                        setFilteredProducts(newfilteredProducts)
                                    }}
