...
const AmenityViewstock = () => {
    ...
    const [amenityStocks, setamenitysStocks] = useState([])
    const [filteramenitysStocks, setFilteramenitysStocks] = useState([])
    ...
    const { data: amenitys, isLoading, isFetching } = useQuery(
    ...
    useEffect(() => {
        if (amenitys !== undefined && amenitys.data && amenitys.data.inventories && amenitys.data.inventories.length > 0) {
            setamenitysStocks(amenitys.data.inventories)
            let units = amenitys.data.inventories.map(item => item.unit)
    ...
    const handleFiltersChange = (filterSelection) => {
        ...
        setFilteramenitysStocks(updatesFilter)
    ...
    return (
        <AuthGuard>
            <DashboardLayout isLoading={isLoading || isFetching}>
                <Head>
                    <title>Dashboard: amenity Stock</title>
                </Head>
                <Box
                    component="main"
                    sx={{
                                    ...
                                    {amenitys.data?.photo ? (
                                        <Box
                                            sx={{
                                                ...
                                                backgroundImage: `url("${amenitys.data?.photo}")`,
                        ...
                        <Card>
                            ...
                            <Scrollbar>
                                <Table sx={{ minWidth: 1200 }}>
                                    ...
                                    {!isEmptyFilter && filteramenitysStocks.length > 0 ? <TableBody>
                                        {filteramenitysStocks.map((inventory) =>
                                    ...
                                    </TableBody> : amenityStocks.length > 0 &&
                                    <TableBody>
                                        {amenityStocks.map((inventory) =>
                                            <Fragment key={inventory?.id}>
                                                <TableRow hover>
                                                    <TableCell width="30%">
                                                        <Box
                                ...
                                {isEmptyFilter && amenityStocks.length === 0 &&
                            ...
                            {amenitys !== undefined && amenitys.data && amenitys.data.inventories && amenitys.data.inventories.length > 0 &&
                                <TablePagination
                                    component="div"
                                    count={filteramenitysStocks.length > 0 ? filteramenitysStocks.length : amenitys.data.inventories.length}
...
export default AmenityViewstock
