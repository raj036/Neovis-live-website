...
import { UnittypeListFilter } from "../../../../components/dashboard/property/unittypes/unittype-list-filters";
...
const UnitTypeList = () => {
  ...
  const { data, refetch, isLoading, isFetching } = useQuery("allUnitType", () =>
    customInstance.get(
      `unit-types?limit=${rowsPerPage}&page=${
        page + 1
      }&search=${searchText}&filter.property_id=${propFilters}&filter.unit_class=${classFilters}&filter.status=${staFilters}`
    )
  );
  ...
  const handleFiltersChange = (filterSelection) => {
    ...
    if (filterSelection?.class?.length > 0) {
      setClassFilters(`$in:${filterSelection.class?.join(",")}`);
    } else {
      setClassFilters("");
    }
          ...
          <Container maxWidth="xl">
            <Box sx={{ mb: 4 }}>
              <Grid container justifyContent="space-between" spacing={3}>
                <Grid item>
                  <Typography variant="h4">Unit Types</Typography>
                </Grid>
                <Grid item>
                  <Button
                    component="a"
                    ...
                    onClick={() =>
                      router.push(
                        "/dashboard/properties/unittypes/newunittype/add"
                      )
                    }
                  >
                    Add
            ...
            <Card>
              <Card>
                ...
                <UnitTypeListTable
                  ...
                  page={page}
                  unittypes={data ? data?.data?.data : []}
                  unittypesCount={data ? data?.data?.meta?.totalItems : 0}
